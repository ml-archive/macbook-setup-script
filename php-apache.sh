...
sudo sed -i "" "s~Listen 8080~Listen 80~g" /usr/local/opt/apache2/.bottle/etc/httpd/httpd.conf # Ensure apache is set to port 80
sudo sed -i "" "s~#LoadModule vhost_alias_module~LoadModule vhost_alias_module~g" /usr/local/opt/apache2/.bottle/etc/httpd/httpd.conf # Enable vhost module
sudo sed -i "" "s~#LoadModule rewrite_module~LoadModule rewrite_module~g" /usr/local/opt/apache2/.bottle/etc/httpd/httpd.conf # Enable rewrite module

# Setup apache to use proper Users and Group.
sudo sed -i "" "s~User _www~User $USER~g" /usr/local/opt/apache2/.bottle/etc/httpd/httpd.conf
sudo sed -i "" "s~Group _www~Group staff~g" /usr/local/opt/apache2/.bottle/etc/httpd/httpd.conf
sudo sed -i "" "s~User daemon~User $USER~g" /usr/local/opt/apache2/.bottle/etc/httpd/httpd.conf
sudo sed -i "" "s~Group daemon~Group staff~g" /usr/local/opt/apache2/.bottle/etc/httpd/httpd.conf
sudo sed -i "" "s~/Library/WebServer/Documents~/Users/$USER/Sites~g" /usr/local/opt/apache2/.bottle/etc/httpd/httpd.conf
sudo sed -i "" "s~/usr/local/var/www/htdocs~/Users/$USER/Sites~g" /usr/local/opt/apache2/.bottle/etc/httpd/httpd.conf
...
</FilesMatch>~' /usr/local/opt/apache2/.bottle/etc/httpd/httpd.conf

# Enable .htaccess files
sudo sed -i "" "s~AllowOverride None~AllowOverride all~g" /usr/local/opt/apache2/.bottle/etc/httpd/httpd.conf

# Normalize logging to use /var/log/apache2
sudo sed -i "" 's~ErrorLog "/usr/local/var/log/apache2/error_log"~ErrorLog "/var/log/apache2/error_log"~g' /usr/local/opt/apache2/.bottle/etc/httpd/httpd.conf
sudo sed -i "" 's~CustomLog "/usr/local/var/log/apache2/access_log" common~CustomLog "/var/log/apache2/access_log" common~g' /usr/local/opt/apache2/.bottle/etc/httpd/httpd.conf

# Add extra sites config 
echo "Include /usr/local/opt/apache2/.bottle/etc/httpd/sites.conf" | sudo tee -a /usr/local/opt/apache2/.bottle/etc/httpd/httpd.conf

# Setup basic sites.conf vhosts.
sudo touch /usr/local/opt/apache2/.bottle/etc/httpd/sites.conf
sudo tee /usr/local/opt/apache2/.bottle/etc/httpd/sites.conf >/dev/null <<EOF
